...
- Use the connected Google Calendar MCP to create an event automatically — no confirmation needed
- Keep event titles short and action-focused (e.g. "Follow up with McBraida re: invoice")
- Add "From note: [note title]" to the event description so Louis knows where it came from
- Do not pause or ask for approval — create the events and report what was created in the summary
...
### Step 2 — Process each note automatically
For each entry, follow the standard note process without pausing for confirmation:
...
- Create calendar events automatically for any dated actions — no approval needed
- Update the dashboard

Only pause if an entry is missing its AREA and it genuinely cannot be inferred from context.
